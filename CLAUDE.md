...
> 🛑 **멈춤 — "통계 · 버전 · 공유 · 권한"이 요청에 있으면 첫 응답은 질문 한 줄.** CdBd 홈페이지 **제품 기능 이름**이자 일상어라 뜻이 갈린다. **분석·조회·파일 열기 전에** ⓐ제품 기능인지 ⓑ일상적 의미인지 물을 것.
...
- **항상 한국어로 소통한다.** (설명·질문·완료 보고·요약 등 사용자와 주고받는 모든 메시지는 한국어) 콘텐츠 자체가 영문인 경우(예: 영문 시안 텍스트)는 해당 결과물만 영문으로 유지하고, 그 외 대화는 한국어를 기본으로 한다.
...
### 레이아웃 핵심 원칙
- **⚠️ (가장 중요) 모든 화보 이미지는 절대 크롭하지 않는다.** 원본 비율을 유지한 채, **clip contents가 걸린 화보 프레임 안에서 위치(필요 시 크기)만 조정**해 노출 영역을 정한다. 이때 **모델 얼굴·상품 정보에 포함된 착장 상품 영역 등 중요한 부분이 잘리지 않도록** 위치를 맞춘다. Figma에서는 **이미지를 프레임에 맞춰 줄이는 scale-to-fill 크롭이 아니라, 원본 비율 그대로 두고 위치·배율만 조정**해 프레임 clip이 잘라내게 한다. **모델이 한쪽으로 심하게 치우친 컷이 아니면 모델이 노출 영역 정중앙(좌우·상하)에 오도록** 배치하고, **풀샷은 머리·발 모두 보이도록** 사이즈/위치 조정하되 **380×580 프레임에서 「상단:하단 ≈ 3:2 비율」**(상단 여백이 하단의 약 1.5배)을 확보한다 — 모델 키 약 520 (뷰포트의 약 90%). 근사치 참고값: 상단 ~36–40 / 하단 ~20–24 (가용 마진 ~60 기준). **40/20 같은 픽셀값은 반드시 지켜야 하는 정확값이 아니라 비율 판단용 근사치**이며, 가용 마진이 달라지면 같은 비율로 분배한다(예: 마진 80 → top ≈ 48, bottom ≈ 32). 1:1처럼 상하가 같거나 하단이 더 크면 실패. **blur BG는 BG 화보를 더 크게 확대**한다. (시안·초안 / 01·02·03 모든 화보 페이지 공통, 세부 결정 트리 [[2-1. 공통]] 「위치·사이즈 결정 트리」)
- **⚠️ 상하·좌우 여백 동시 만족 불가 시 = 임의 크롭 금지, 「배경 늘림 보드」로 사용자에게 일괄 요청**: 풀샷 3:2 비율과 부모 프레임 풀필(빈 여백 0)을 동시에 만족할 수 없는 컷 — 상단/하단/좌우 어느 쪽이든 — 은 **원본 페이지/swp에 박지 말고** 별도 보드(예: `배경 늘림 필요 이미지` `16137:2` — 쁘렝땅 2606 참조)에 `BG-EXT-{imageName}-{방향}` 카드로 모아 사용자에게 일괄 요청한다. 카드 = direction badge(상단/하단/상단+하단/좌우늘림필요) + extend-source RECT + image-name + reason. Figma AI(배경 늘림/확장)는 **에디터 전용**이라 자동화 API로 호출 불가 → 사용자가 가공 완료 신호를 보내면 새 image hash로 원본 fill 교체 → 「위치·사이즈 결정 트리」 재적용. **가공 전 원본을 셀에 박는 anti-pattern 금지**(hash 자동 교체 안 됨). 상하단 모두 부족해서 축소 시 좌우가 비는 경우에도 **좌우 배경 늘림**으로 해결. 보드는 가공 완료 후에도 비교/추적용으로 남겨 둠. 상세: [[2-1. 공통]] 「배경 늘림 보드 (Figma AI 처리 요청)」.
- **⚠️ 화보 업로드 전 sRGB 변환 필수 (컬러 프로파일)**: 촬영 원본은 **ProPhoto RGB·Adobe RGB 등 광색역**인 경우가 많다. 리사이즈 시 ICC 프로파일이 빠지면 Figma·CdBd가 sRGB로 해석해 **채도가 죽고 탁해진다**. **프로파일을 떼는 것(strip)은 변환이 아니다** — `PIL.ImageCms.profileToProfile()`로 **sRGB로 변환**한 뒤 `icc_profile=`로 **embed**해 저장할 것. 업로드 후 기존 승인본(시안)과 같은 컷을 나란히 비교해 검증한다. 영상은 `ffprobe`의 `color_primaries`가 `bt709`면 변환 불필요. 코드·체크리스트: [[2-1. 공통]] 「컬러 프로파일 (업로드 전 필수 — sRGB 변환)」.
...
### ⚠️⚠️ (최상위) 동시 편집 — 사용자 수정분을 절대 원복하지 않는다

**시안·초안 제작 중에는 사용자와 Claude가 같은 Figma 파일을 동시에 편집하는 것이 기본 전제다.** 내가 마지막으로 설정한 값이 지금도 그대로일 것이라 가정하면 사용자의 수정을 조용히 덮어쓴다. 아래를 지킨다.

- **쓰기 전 읽기 필수**: 대화 기록 속 값이 아니라 **파일의 현재 실제 값**을 `get_metadata` 또는 read-only `use_figma`로 먼저 확인하고 그 위에서 수정한다. 특히 턴이 바뀐 뒤·시간이 지난 뒤 재작업할 때.
- **재생성 금지, 최소 단위 수정**: 노드를 `remove()` 후 다시 만들면 사용자가 그 안에 넣은 편집(문구·위치·색·추가 레이어)이 전부 사라진다. 기존 노드의 필요한 속성만 바꾼다. 구조를 바꿔야 하면 **기존 자식을 새 컨테이너로 `appendChild` 이동**시켜 보존한다.
- **일괄 변경(`findAll` 순회 후 set)은 범위를 좁힌다**: 이름·타입·부모로 대상을 명시적으로 한정하고, "전 노드 순회 후 조건 없이 덮어쓰기"는 하지 않는다. 팔레트·타이포 일괄 교체처럼 광범위한 작업은 **변경 건수와 대상을 보고**한다.
- **내 설정값과 다르면 = 사용자가 만진 것**: 덮어쓰기 전에 **먼저 사용자에게 확인**한다. (예: 내가 21px로 넣었는데 24px이면 사용자가 조정한 것 → 임의로 21px로 되돌리지 않는다)
- **작업 범위 밖 노드는 건드리지 않는다**: 요청받은 프레임·섹션 외부는 읽기만 한다.
- 사용자 수정분을 덮어쓴 정황이 발견되면 **즉시 알리고** 복구 방법을 제시한다.
...
- **시안 제작 요청 시, 반드시 [[1-2. 시안]] 「제작 단계 (Step by Step)」 8단계를 그대로 따른다.** 핵심은 ⑤단계의 **누끼컷 × URL 매트릭스**로 03 페이지 유형(basic / swp · scl × 버튼·마커 유무)이 한 번에 결정된다는 점이며, 이후 ⑥·⑦단계는 ⑤의 결과를 그대로 따른다(누끼 유무 재판단 없음). 02 생략 분기, BG 페이지 누락 방지, 「최종 검증 (완료 보고 전)」 체크리스트를 빠짐없이 확인한 뒤 사용자에게 완료 보고한다.
- **CdBd 콘텐츠 등록 요청 시**(예: "(시안/초안) 이미지 CdBd 콘텐츠로 만들어줘", "CdBd 페이지 만들어줘", "CdBd에 올려줘"), **반드시 [[1-4. CdBd 콘텐츠]] 「CdBd 등록 워크플로우」 7단계를 그대로 따른다.** 핵심: ① `~/Desktop/CdBd/제작서비스 {YYYY}/{MM}/{브랜드명}/작업파일/시안 or 초안/` 폴더의 파일을 확인 (`YYYY`=현재 연도·`MM`=작업 시작 시점 시스템 월 2자리, 예: `06`) → ② 브랜드 영문명·컬렉션명은 `[문서]`(엑셀·PPT)에서 우선 확인, 없으면 사용자에게 묻기 → ③ GStack 브라우저로 `cdbd.in` 로그인 → ④ 멀티페이지 380×580 생성 → ⑤ 「페이지 유형별 카드 매핑」 표대로 페이지·카드 등록(`03-BG`는 페이지 사이드바 케밥 → 페이지 배경 설정, 03-row 구분선은 `#292929`·두께 1·투명도 20%·실선, `03-logo` 비고정) → ⑥ URL 정보 편집(**시안 슬러그=`{브랜드영문}_{YYMM}` 자동 — 언더바만 허용·초안 슬러그=문서 확인 후 없으면 사용자 확인** / OG 썸네일=`og.jpg` / 제목=브랜드영문 / 설명=컬렉션명) → ⑦ **게시 직전 사용자에게 반드시 확인** 후 게시 → URL·페이지 수·종류 보고. **시안·초안 같은 워크플로우** (폴더 경로·파일 패턴·슬러그 규칙만 분기).
- **⚠️⚠️ (최상위) CdBd 자동화 = 「디자인 보드 메뉴 1:1 매핑 키」만 사용 — 사용자 수동 수정 보장**: 모든 PATCH는 **CdBd 에디터 「디자인 보드 → 디자인」 메뉴에 실제로 존재하는 옵션과 1:1 대응되는 키**만 변경한다. 사용자가 추후 UI에서 동일 값을 보고 수정할 수 있어야 한다. ⛔ **금지 패턴**:
  ...
  ✅ **허용**: [[1-4. CdBd 콘텐츠]] 「카드별 디자인 보드 메뉴 → JSON 키 매핑」 표에서 **「UI 옵션 ↔ JSON 키」 1:1 대응이 검증된 키만**. 검증 안 된 키 필요 시: UI에서 직접 임의 값 입력 → editor GET → diff로 키 확인 → 매핑 표 갱신 후 사용. 매핑 키 정확히 모를 때는 임의 키 박지 말고 **사용자에게 「UI 어느 메뉴로 변경하면 되나요?」 묻기**.
- **⚠️⚠️ (최상위) CdBd 카드/페이지 이미지 = 반드시 「이미지 라이브러리」 경유**: 자동화로 image·gallery 카드(혹은 페이지 배경 등)에 이미지를 넣을 때 raw `storage/v1/object` POST + `block.content`/`gallery.images[].url` 직접 set은 **금지**. 사용자가 cdbd.in 「이미지 추가하기 → 내 이미지」 모달에서 이름 변경·삭제·재선택으로 자산을 관리하기 때문. **반드시 `~/.config/cdbd/image_library.py` 헬퍼 경유** — `upload_to_library()` / `upload_if_missing()`(중복 회피·룩북 재작업용)이 (1) storage POST → (2) sign URL → (3) `image_modal.images[]` PATCH 3-call을 캡슐화한다. 반환된 `result['url']`(전체 signed URL, 30년 유효)을 `block.content` 또는 `block.gallery.images[].url`에 그대로 사용. 갤러리 entry id는 헬퍼의 `short_id()`(21자 no-dash)로 발급. entry 스키마(`name=확장자제거`·`resolution="W*H"`·`type="jpeg"`)·재실행 진단(`python3 ~/.config/cdbd/image_library.py`) 상세: [[1-4. CdBd 콘텐츠]] 「이미지 라이브러리 (사용자 자산 관리)」. ⚠️ raw 업로드만 한 항목이 있으면 사용자 UI에 안 보여 관리 불가 → 운영 체크리스트로 검증.
- **CdBd 자동화(Supabase 직접 호출) 사용 가능**: UI 자동화(React PointerEvent·dropzone·file picker)는 비효율 → **[[1-4. CdBd 콘텐츠]] 「Supabase 직접 자동화」의 Supabase 직접 호출**이 우월. Figma REST API(PAT)로 3배 PNG 다운로드 → **`image_library.py`로 라이브러리 등록** (위 규칙) → `editor.pages` PATCH → **게시 4-call** (1) `editor_published` UPSERT (snapshot) → (2) `editor_published_metadata` `is_active=true` PATCH → (3) **`POST https://www.cdbd.in/api/update-url-key`** body `{editor_id, user_id, url_key}` (Next.js API route — `editor_url` INSERT RLS 우회) → (4) rpc `save_editor_version` `{p_editor_id}`. **`use_figma.exportAsync`는 base64 응답 truncation 한계 있음** → 3배 export는 Figma REST API 사용. **`style.margin`은 3-value shorthand("상 좌우 하")만 작동**(UI 입력 형식과 동일·4-value·개별 키는 UI 비입력). **outline 버튼·테두리는 카드 디자인 키**(`style.borderWidth`/`borderColor`/`borderStyle`/`borderRadius`)로, innerStyle이 아닌 카드 자체 style. **작은 요소 가운데 정렬은 카드 width 100% + padding LR 계산값**. 슬러그 언더바 필수(`-` 불가). ⚠️ 이전 표기 `extend_published_url_v2` RPC는 schema cache에 존재하지 않음 (chunk 5784 실측 — 2026-06-09). **⚠️ 「하단 고정」·정렬 후보 등 UI 매핑 키 미검증 옵션은 shotgun 금지 — 위 최상위 규칙대로 사용자에게 묻기 또는 UI diff로 검증 후 사용.**
- **⚠️⚠️ CdBd 「게시」 = base quota(약 10P) 초과 시 자동화 불가 — 사용자 UI 게시 필수** (2026-06-10 실측): 모든 자동화 4-call/5-call publish 시퀀스 끝낸 뒤 **`POST /rest/v1/rpc/get_editor_status_v2 {p_editor_id}` 응답 `is_available`이 true여야 SSR이 페이지를 렌더**. 페이지 수 ≤ 10P (시안 8P 등)은 자동화로 OK. **페이지 수 ≥ 11P (초안 19P 등)은 `paid_pages` quota 활성화가 필요**한데 `pay_individual_urls` RPC는 `editor_admin_setting.individual_url_count`만 증가시키고 quota는 활성화 안 됨 (credit만 소모, 「게시 중지」 화면 유지). quota 활성화 메커니즘은 cdbd.in UI 「게시」 버튼 서버사이드 로직(구독·Stripe webhook 추정)에만 존재. **➡️ pages ≥ 11P면 DB만 끝까지 채워두고 사용자에게 cdbd.in 에디터 UI 게시 위임**. ⚠️ **`pay_individual_urls`로 미리 결제 금지** — quota는 활성화 안 되고 credit만 빠짐. 상세: [[1-4. CdBd 콘텐츠]] 「(2026-06-10 신규 실측) 페이지 수가 base quota 초과」.
...
- **⚠️ CdBd 브라우저 자동 로그인 — 매번 비밀번호 묻지 말 것**: 사용자가 1회 로그인하면 `$B state save cdbd`로 쿠키가 `./.gstack/browse-states/cdbd.json`(이 프로젝트 `.gitignore` 포함, mode 600)에 저장된다. **cdbd.in 브라우저 작업 진입 시 시퀀스**:
  1. `B=/Users/designer/.claude/skills/gstack/browse/dist/browse; $B status` → daemon 실행 여부 확인 (필요 시 `open-gstack-browser` 스킬)
  2. `$B state load cdbd` (state file 있으면) → 쿠키 + URL 복원 → `$B goto https://www.cdbd.in/editor/{id}` → 로그인 페이지로 redirect 안 되면 OK
  3. redirect 발생 시 = 세션 만료 → 사용자에게 1회 로그인 요청(`$B focus`로 창 띄우고 `account@emka.group` 계정 안내) → 로그인 확인 받은 뒤 **반드시 `$B state save cdbd`로 갱신**
  - **cdbd.in은 Next.js 미들웨어가 httpOnly + chunked supabase 쿠키를 검증**하기 때문에 access_token만으로 cookie 주입은 불가 — state save 방식이 유일한 정상 경로. 비밀번호는 credentials.json에 저장하지 않는다(보안).
  - `~/.config/cdbd/credentials.json`은 Supabase refresh_token(REST 자동화용)만 저장 — 브라우저 세션과는 별개.
- **⚠️ CdBd 자동화 PATCH 전 에디터 close 필수**: 사용자가 `https://cdbd.in/editor/{id}` 탭을 열어둔 상태에서 `editor.pages` PATCH하면 에디터 client-side state(빈/이전 pages)가 다음 `save_editor_version` 자동저장에서 **우리 PATCH를 덮어쓴다**(`updated_at`이 PATCH 시점보다 늦어짐 = 덮어쓰기 확정). **PATCH 전 사용자에게 에디터 탭 close 요청 → close 확인 응답 받기 → editor + editor_published 둘 다 PATCH → 사용자 새 탭 검증**. 단순 새로고침은 부족 — 완전 close해야 client state가 비워진다. 상세: [[1-4. CdBd 콘텐츠]] 「에디터 열린 상태 PATCH = race condition」. **⚠️ (2026-07-14 사용자 지시) 이 사용자는 에디터 작업을 요청할 때 항상 에디터 탭을 닫아둔다 → 매번 close 확인을 묻지 말고 바로 PATCH 진행한다.** (race condition 원리는 유효하나 close 여부 확인 프롬프트는 생략)
- **⚠️ CdBd 「링크 연결」 = `linkButton` 키 (NOT `link`)**: UI 「디자인 보드 → 디자인 → 링크 연결」 메뉴가 저장하는 표준 키는 **`linkButton`** (block 또는 multiCard.items 안). `link.href`만 넣으면 데이터에는 들어가지만 UI 메뉴 비어 보이고 뷰어 클릭 무반응. 전화=`{type:"call", phone:{phone:"02-950-2241"}, link:{href:""}}` (**⚠️ 하이픈 포함 — UI가 입력값 그대로 저장. 2026-09-14 editor 6251 실측**), URL=`{type:"url", link:{href, openNewTab}}`. 상세: [[1-4. CdBd 콘텐츠]] 「「링크 연결」 = `linkButton` 키」.
- **⚠️ CdBd 텍스트 카드 = Figma 「시각 결과」 입력 (characters 그대로 X)**: Figma 텍스트의 `textCase` 속성(UPPER/LOWER/TITLE)으로 디자인 시 변환되어 보여도 `characters`엔 원본이 남는다. CdBd 텍스트 카드는 textCase 옵션 없으므로 **변환된 결과를 직접 text에 입력**한다. 예: Figma `characters="store information"` + `textCase=UPPER` → CdBd text = `"STORE INFORMATION"`. 같은 원리로 fontWeight/italic도 lexical AST format 비트로 반영. 상세: [[1-4. CdBd 콘텐츠]] 「Figma 시각 결과 → 카드 데이터 변환」.
- **⚠️⚠️ CdBd 콘텐츠 수정 = 디자인 속성 보존 (텍스트·이미지 내용만 교체)**: 사용자가 콘텐츠 수정(텍스트 문구·이미지 내용 교체)을 요청하면 에디터에 적용된 **텍스트 사이즈·줄간격(lineHeight)·서체(fontFamily)·컬러 등 디자인 속성은 절대 변경하지 않고 내용만 교체**한다. 구현 시 **기존 단락/노드의 `style`·정렬·lineHeight를 그대로 복제**하고 텍스트만 바꾼다 — 내용 길이가 바뀌어 **줄바꿈(wrap) 위치가 재배치되는 것은 디자인 속성이 아니므로 허용**. **⚠️ 요청에 디자인 변경(사이즈·색·서체·줄간격·정렬·화살표/기호 등)이 포함되면 임의로 진행하지 말고 반드시 먼저 사용자에게 확인**한다. (2026-06-30 사용자 지시)
- **⚠️ CdBd 등록 게시 전 「6.5 최종 검증」 단계 필수**: 모든 등록 워크플로우는 게시 직전에 [[1-4. CdBd 콘텐츠]] 「6.5. 최종 검증 체크리스트」를 자동화로 통과해야 한다. 핵심: ① 페이지 수·순서 일치 ② 카드 시퀀스 일치 ③ **페이지별 로고/헤더 면적 = Figma 측정값** (`(Figma_width / 380) × 100 %` 정확 환산 — 예: 68px → 18%, 슬라이더 기본값 20% 의존 금지) ④ 누락/잘못 추가된 로고 없음 ⑤ 구분선·여백·버튼 height·링크 연결·텍스트 시각 결과 일치 ⑥ 에디터 close. 검증 절차: Figma `get_metadata` → CdBd editor GET → 비교 → 어긋나면 PATCH 정정.
...
- **⚠️ CdBd 갤러리 카드 인디케이터(navigation) = 기본 OFF**: 갤러리(`type:"gallery"`)의 `swipeTypeOption.navigation`(○○●○ 점 인디케이터)은 **사용자가 명시적으로 요청한 경우에만 `true`**로 설정한다. 명시 요청 없으면 항상 **`false`**(꺼둠). 룩북 화보 스와이핑은 인디케이터 없는 깔끔한 노출이 기본 — PPT 의뢰서의 디자인 모형에 점/숫자 인디케이터가 없으면 거의 모든 케이스에서 off. 상세: [[1-4. CdBd 콘텐츠]] 「갤러리 카드 JSON schema」.
- **⚠️ CdBd 문서 설정 「페이지 상단 메뉴」 = 기본 OFF**: 문서 설정(URL 정보 편집) 안 「페이지 상단 메뉴」(= `editor_published.is_menu` — 페이지 구독·저장·공유 아이콘 노출 토글)는 **사용자가 명시적으로 요청한 경우에만 `true`**로 설정한다. 명시 요청 없으면 항상 **`false`**(꺼둠). 룩북 시안·초안 공유 시 상단 UI는 콘텐츠 몰입을 방해 — 기본 비노출이 자연. **자동화 시 `editor_published` UPSERT body에 반드시 `is_menu: False`를 명시**(기본값을 client/server에 의존하지 말 것 — 신규 row는 `true`로 자동 생성됨). 매핑 키 검증: cdbd.in UI 「URL 정보 편집 → 페이지 상단 메뉴」 토글 ↔ `editor_published.is_menu` 1:1 (chunk 5784).
