...
- **⚠️ CdBd 갤러리 카드 인디케이터(navigation) = 기본 OFF**: 갤러리(`type:"gallery"`)의 `swipeTypeOption.navigation`(○○●○ 점 인디케이터)은 **사용자가 명시적으로 요청한 경우에만 `true`**로 설정한다. 명시 요청 없으면 항상 **`false`**(꺼둠). 룩북 화보 스와이핑은 인디케이터 없는 깔끔한 노출이 기본 — PPT 의뢰서의 디자인 모형에 점/숫자 인디케이터가 없으면 거의 모든 케이스에서 off. 상세: [[룩북/1. 제작 프로세스/1-4. CdBd 콘텐츠.md]] 「갤러리 카드 JSON schema」.
