...
- **⚠️ CdBd 「링크 연결」 = `linkButton` 키 (NOT `link`)**: UI 「디자인 보드 → 디자인 → 링크 연결」 메뉴가 저장하는 표준 키는 **`linkButton`** (block 또는 multiCard.items 안). `link.href`만 넣으면 데이터에는 들어가지만 UI 메뉴 비어 보이고 뷰어 클릭 무반응. 전화=`{type:"call", phone:{phone:"숫자만, 하이픈X"}}`, URL=`{type:"url", link:{href, openNewTab}}`. 상세: [[1-4. CdBd 콘텐츠]] 「「링크 연결」 = `linkButton` 키」.
