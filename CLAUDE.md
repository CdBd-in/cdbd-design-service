...
작업 전 반드시 해당 문서를 확인하고, 변경 시 함께 갱신합니다. 문서는 **작업 프로세스**(`제작 프로세스/`)와 **디자인 가이드**(`디자인 가이드/`)로 분리되어 있습니다.

**`룩북/제작 프로세스/`** — 구성·파일명·내보내기 등 작업 프로세스
| 문서 | 내용 |
|------|------|
| `제작 프로세스/공통.md` | 페이지 규격(380×580), OG 이미지, 해상도·파일 형식·파일명·저장위치 규칙 |
| `제작 프로세스/시안.md` | 시안 프로세스 (A/B 2안 채택, 4×2=8페이지 구성, 시안 표지, 체크리스트) |
| `제작 프로세스/초안.md` | 초안 프로세스 (전체 페이지 흐름·체크리스트, 디자인은 선택된 시안 승계) |

**`룩북/디자인 가이드/`** — 페이지 유형별 디자인 규칙
| 문서 | 내용 |
|------|------|
| `디자인 가이드/0. 공통.md` | 화보 비크롭, 기본 폰트(Pretendard), border radius, 01·02 핵심 원칙·공통 텍스트/로고 스펙 |
| `디자인 가이드/1. 표지.md` | 01 표지 + 표지 레이아웃 타입(thin/clean/thick frame, blur BG) |
| `디자인 가이드/2. 브랜드·컬렉션 소개.md` | 02 브랜드·시즌 소개(dim 오버레이, 02-A~D) + 표지 레이아웃 타입 |
| `디자인 가이드/3. 화보·상품.md` | 03 내지 (basic / swp / scl) |
| `디자인 가이드/4. 매장 정보.md` | 매장·연락처 페이지 (빈 골격, 추후 작성) |
...
- **프로세스 규칙은 `제작 프로세스/`, 디자인 규칙은 `디자인 가이드/`에 둔다.** 규칙을 변경하면 두 폴더 문서 간 충돌·위계·용어 일치를 점검한다.
