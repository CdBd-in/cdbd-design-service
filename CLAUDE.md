...
- **이미지 가공(누끼 배경 제거 등)은 Figma AI(`Remove background`)를 사용**한다 — 로컬 rembg/BiRefNet 등 외부 ML은 쓰지 않는다. ⚠️ 단 **Figma AI는 에디터 전용 기능이라 자동화 API(`use_figma`)로는 호출 불가**(`createImage`/`getImageByHash`만 존재). → 원본을 Figma에 올린 뒤 **에디터에서 `Remove background`를 적용**(필요 시 사용자에게 요청)한 결과로 진행한다.
