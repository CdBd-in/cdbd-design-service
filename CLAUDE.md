...
- **swp 셀 구조 = VERTICAL auto-layout** (`primaryAxis = SPACE_BETWEEN`, `counterAxis = CENTER`, padding 0, itemSpacing 0). 자식 2개: ① 이미지 RECTANGLE (FIXED, 셀 상단·좌우 중앙) ② 텍스트 wrapper FRAME (VERTICAL hug auto-layout, `counterAxis = CENTER`, `primaryAxis = MAX`, itemSpacing 0) → 이름·가격 2개를 hug로 묶어 셀 하단·좌우 중앙에 자동 anchor. 결과: 텍스트 박스는 **셀 하단 여백 없이·좌우 중앙**, 이미지는 **셀 상단·좌우 중앙** (auto-layout이 위치 보장). 이전 절대 좌표 패턴(이미지 y 직접 계산 + 텍스트 y=cell.h - text.h)은 deprecated. ⚠️ auto-layout 적용 후 셀이 hug로 줄면 `cell.resize(cellW, cellH)`로 명시 재고정. **자식 위치 권장**: rect는 `layoutSizingHorizontal/Vertical = "FIXED"`, wrapper는 `"HUG"`.
...
- **CdBd 자동화(Supabase 직접 호출) 사용 가능**: UI 자동화(React PointerEvent·dropzone·file picker)는 비효율 → **[[룩북/1. 제작 프로세스/1-4. CdBd 콘텐츠.md]] 「Supabase 직접 자동화」의 Supabase 직접 호출**이 우월. Figma REST API(PAT)로 3배 PNG 다운로드 → Supabase Storage POST → `editor.pages` PATCH → `rpc/extend_published_url_v2`로 슬러그 저장. **`use_figma.exportAsync`는 base64 응답 truncation 한계 있음** → 3배 export는 Figma REST API 사용. **`style.margin`은 3-value shorthand("상 좌우 하")만 작동**(UI 입력 형식과 동일·4-value·개별 키는 UI 비입력). **outline 버튼·테두리는 카드 디자인 키**(`style.borderWidth`/`borderColor`/`borderStyle`/`borderRadius`)로, innerStyle이 아닌 카드 자체 style. **작은 요소 가운데 정렬은 카드 width 100% + padding LR 계산값**. 슬러그 언더바 필수(`-` 불가). 게시 시퀀스 12 호출. **⚠️ 「하단 고정」·정렬 후보 등 UI 매핑 키 미검증 옵션은 shotgun 금지 — 위 최상위 규칙대로 사용자에게 묻기 또는 UI diff로 검증 후 사용.**
- **⚠️ CdBd 자동화 진입 시 인증 헬퍼 자동 사용**: 매번 사용자에게 token 요청하지 말 것. **`~/.config/cdbd/auth.py`의 `get_credentials()`** 함수가 figma_pat + 최신 access_token(refresh_token으로 자동 발급) + user_id를 한 번에 반환한다. 사용 패턴: `import sys; sys.path.insert(0, '/Users/leesunho/.config/cdbd'); from auth import get_credentials; creds = get_credentials()`. credentials.json 없거나 refresh 실패 시에만 사용자에게 안내 (cdbd.in 재로그인 후 refresh_token 재추출). **Supabase access_token은 1시간 만료지만 refresh_token으로 자동 갱신되므로 사용자 알림 불필요**. Figma PAT 90일 만료만 사용자에게 미리 알릴 것.
