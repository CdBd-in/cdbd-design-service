...
| `1. 제작 프로세스/4-CdBd 콘텐츠.md` | 내보내기 파일 ↔ CdBd 카드 매핑 (룩북 사용 카드 3종: 이미지·갤러리[나열하기/넘겨보기/그리드]·구분선) + 멀티페이지 구조·카드별 옵션 명세·페이지 유형별 속성 규칙 |
| `1. 제작 프로세스/5-CdBd 자동화.md` | **개발자/AI용** Supabase 직접 호출 자동화 경로 — 백엔드 인프라(URL·anon key·access token), 테이블/RPC(`extend_published_url_v2`), `editor.pages` PATCH 패턴, CdBd JSON 카드 spec(margin 3-value·테두리·fill rgba·하단 고정·중앙 정렬), Figma REST API(PAT) 자산 파이프라인, 슬러그 언더바 규칙 등 |
...
- **누끼 그리드 통일감**: 그리드 **단 수(2단/3단)와 무관하게 누끼 이미지의 가로×세로 면적이 비슷하도록** 정규화한다(2단이라고 크고 3단이라고 작아지지 않게). **같은 행 상품끼리는 상하 중앙 정렬**, **2단(2개) 행은 상품 간 간격을 더 넓게** 둔다. 전체 상품 크기는 카드 여백이 답답하지 않게 **과하지 않은 크기**로. **같은 라인 상품을 한 행에 배치할 때 = 좌측 상의(top), 우측 하의(bottom) 순서 고정** (예: Jacket 좌 + Pants 우, Cropped Jkt 좌 + Cotton Short 우).
- **swp 페이지 2행 그리드 = 카드 내 vertical center**: swp 페이지에 2행만 배치되는 경우(예: swp3 = 4 product groups), 카드 내에서 **상하 균등 여백으로 수직 중앙 정렬** (어느 한쪽 치우침 없음).
...
- **CdBd 자동화(Supabase 직접 호출) 사용 가능**: UI 자동화(React PointerEvent·dropzone·file picker)는 비효율 → **[[룩북/1. 제작 프로세스/1-5. CdBd 자동화.md]]의 Supabase 직접 호출**이 우월. Figma REST API(PAT)로 3배 PNG 다운로드 → Supabase Storage POST → `editor.pages` PATCH → `rpc/extend_published_url_v2`로 슬러그 저장. **`use_figma.exportAsync`는 base64 응답 truncation 한계 있음** → 3배 export는 Figma REST API 사용. **`style.margin`은 3-value shorthand("상 좌우 하")만 작동**(4-value·개별 키 무시). **outline 버튼·테두리는 카드 디자인 키**(`style.borderWidth`/`borderColor`/`borderStyle`/`borderRadius`)로, innerStyle이 아닌 카드 자체 style. **하단 고정(`fixed`/`pinned`/`sticky`/`isFixed`/`position`/`fixedPosition` 모두 true 설정)**, **작은 요소 가운데 정렬은 카드 width 100% + padding LR 계산값**. 슬러그 언더바 필수(`-` 불가). 게시 시퀀스 12 호출.
