...
- **참조 룩북 문서**: [[1-4. CdBd 콘텐츠]] (카드 매핑·image_library·linkButton 규칙)
...
총 **4P / 30 카드** → base quota 10P 이내라 자동화 publish 가능 ([[1-4. CdBd 콘텐츠]] 「base quota 초과」 규칙 통과).
...
**규칙 적용**: 사용자가 [[2-1. 공통]] 「화보 = 별도 자식 RECTANGLE 레이어」 강제 → 모든 프레임에서 `frame.fills = []`, 자식 RECT로 이미지 이전. `clipsContent = true`. 각 RECT를 Figma에서 자유롭게 위치·사이즈 조정 가능.
...
- [[1-1. 공통]] — 내보내기 체크리스트 통합 (JPG·PNG)
- [[1-2. 시안]] — swp 정의 마스터 + 작업 체크리스트 + 최종 검증 위계 분리
- [[1-3. 초안]] — 작업 체크리스트 + 최종 검증 위계 분리
- [[1-4. CdBd 콘텐츠]] — 6.5 최종 검증 단일 진입점 + 자동화·이미지 라이브러리 체크리스트 명명 통일
