...
**[[룩북/2. 디자인 가이드/2-1. 공통.md]] 「화보 이미지 배치」** 안에 신설 서브섹션:
...
**갱신 위치**:
- [[룩북/2. 디자인 가이드/2-1. 공통.md]] 결정 트리 [규칙 A] + 빠른 참조 표 + 「풀샷 (Rule A) 상단:하단 ≈ 3:2 비율 계산」 섹션 전면 갱신
- [[룩북/2. 디자인 가이드/2-4. 화보·상품.md]] 03-BG 풀샷 마진 갱신
...
- [[룩북/2. 디자인 가이드/2-1. 공통.md]] — 「배경 늘림 보드」 신설 + 「풀샷 (Rule A) 상단:하단 ≈ 3:2 비율 계산」 갱신 + 결정 트리·빠른 참조 표 갱신
- [[룩북/2. 디자인 가이드/2-4. 화보·상품.md]] — 03-BG 풀샷 마진 3:2 비율 갱신 + 배경 늘림 보드 참조
- [[룩북/1. 제작 프로세스/1-4. CdBd 콘텐츠.md]] — paid_pages quota 경고 + 복제 우회 패턴 + `get_editor_status_v2` 진단 RPC + `/api/create-admin-setting` 발견
